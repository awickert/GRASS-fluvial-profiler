...
def get_xEN(cat, x0=0., streams=streams):
    ...
    dx_downstream = ( (_diffs**2).sum(axis=1) )**.5
    x_downstream = np.concatenate( [[0], np.cumsum(dx_downstream)] )
    x_upstream = x_downstream[-1] - x_downstream

    data.close()
    
    return x_downstream, x_upstream, E, N

def get_xEN_all(streams=streams):
    """
    asdf
    """
    ...
    for _cat in cats:
        ...
        for _c in coords:
            ...
            with RasterRow("dem", mode="r") as rast:
                _reg = region.Region()
                z_list.append(rast.get_value((x, y), _reg))
        z_lol.append(z_list)
    df_edges['z'] = z_lol

# Generalize the above to any raster
