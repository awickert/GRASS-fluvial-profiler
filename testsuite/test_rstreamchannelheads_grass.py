"""
GRASS-inclusive tests for r.stream.channelheads.

Need a live GRASS session; run locally (not in CI). Require r.stream.channelheads
on the GRASS addon path and rivernetworkx importable. Note this module does NOT
...
topology). Run with, e.g.:
    ...
    MPLBACKEND=Agg PYTHONPATH=. GRASS_ADDON_BASE=/tmp/rnx-addon \
        ...
        testsuite/test_rstreamchannelheads_grass.py
"""
...
class TestChannelHeads(TestCase):
    """DEM -> r.watershed -> r.stream.extract -> r.stream.channelheads (no v.stream.network)."""
    ...
    def tearDownClass(cls):
        ...
        cls.runModule('g.remove', flags='f', type='vector', name='streams,heads')
        cls.del_temp_region()

    def test_finds_heads_without_topology(self):
        # -c: the synthetic catchment drains off-map, so treat negative
        # accumulation at heads as a boundary artifact. window=0 = no smoothing.
        self.assertModule('r.stream.channelheads', flags='c', elevation='dem',
                          accumulation='accum', streams='streams', output='heads',
                          window=0, min_slope=1e-9, overwrite=True)
        self.assertVectorExists('heads')
        topo = gscript.vector_info_topo('heads')
        self.assertGreater(int(topo['points']), 0)       # produced head points
        cols = gscript.vector_columns('heads')
    ...
    def test_offmap_without_flag_fails(self):
        ...
        self.assertModuleFail('r.stream.channelheads', elevation='dem',
                              accumulation='accum', streams='streams',
                              output='heads_bad', window=0, min_slope=1e-9)
