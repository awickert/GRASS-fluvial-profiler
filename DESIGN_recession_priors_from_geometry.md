...
2. **Where do channels begin? (the channel head / hillslope–fluvial
   ...
   1988/1992; Tarboton). GFP already computes slope–area in `v.stream.profiler`,
...
DEM
 ...
 └─ r.stream.extract                    → streams (raster+vector), draindir
       └─ [channel-head threshold]       ← slope–area (v.stream.profiler) — sets extent
...
|---|---|---|---|
| **Geomorphic** | profiles, k_sn / χ steepness, knickpoints, slope–area, drainage density, landscape-evolution setup | `v.stream.network(x)`, `v.stream.profiler` | (steepness/χ are on the README roadmap) |
