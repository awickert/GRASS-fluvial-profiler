...
def main():
        ...
        for i in range(len(data)):
            if type(data.read(i+1)) is vector.geometry.Line:
                if data.read(i+1).cat in selected_cats:
                    ...
                    print float(_i)/float(len(selected_cats))
                    gscript.core.percent(_i, len(selected_cats), 1./len(selected_cats))
                    _i += 1
    ...
    if options['elevation']:
        ...
        #r = region.Region()
        #r.west = 525800; r.east = 534300; r.north = 4926000; r.south = 4920000
        #DEM.set_region(r)
        DEM.open('r')
        z = []
        _i = 0
