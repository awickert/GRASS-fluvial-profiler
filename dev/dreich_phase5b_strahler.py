"""Phase 5b: valley junctions at equal-order (Strahler) merges, via a Python
Strahler pass on the stream network (avoids the r.stream.order hang)."""
import numpy as np
...
A = np.abs(acc); fd = np.where(np.isfinite(fd), fd, -1.0)
...
for d, (dr, dc) in move.items():
    ...
    inb = (tr >= 0) & (tr < nr) & (tc >= 0) & (tc < nc); tr, tc = tr[inb], tc[inb]
    st = chan[tr, tc]; np.add.at(inflow, (tr[st], tc[st]), 1)
# Python Strahler on the stream network (Kahn from sources)
order = np.zeros(dem.shape, np.int16); maxin = np.zeros(dem.shape, np.int16)
cntmax = np.zeros(dem.shape, np.int16); indeg = inflow.copy()
src = [(int(r), int(c)) for r, c in zip(rr, cc) if inflow[r, c] == 0]
q = deque(src)
for s in src:
    order[s] = 1
while q:
    u = q.popleft(); nb = dn(*u)
    if nb is None or not chan[nb]:
        continue
    o = order[u]
    if o > maxin[nb]:
        maxin[nb] = o; cntmax[nb] = 1
    elif o == maxin[nb]:
        cntmax[nb] += 1
    indeg[nb] -= 1
    if indeg[nb] == 0:
        order[nb] = maxin[nb] + (1 if cntmax[nb] >= 2 else 0)
        q.append(nb)
print('coarse sources: %d ; max Strahler order: %d' % (len(src), int(order.max())))

def is_so_junction(c):              # the down-neighbor has higher Strahler order
    nb = dn(*c)
    return nb is not None and order[nb] > order[c]
...
def to_second_order_junction(r, c, maxlen=2000):
    ...
    for _ in range(maxlen):
        ...
        if nb is None:
            break
        prev = (r, c); path.append(nb); r, c = nb
        if order[r, c] > order[prev]:
            jumps += 1
            if jumps >= 2:
                break
    return path
...
print('Phase5b port heads: %d  (C++ reference 634)' % len(heads))
ref = np.load('/tmp/dreich_ref_heads.npy')
d = np.array([np.min(np.hypot(ref[:, 0] - e, ref[:, 1] - n)) for e, n in zip(E, Nn)])
print('Phase5b-vs-reference (Euclidean): median %.1f m | <=5m %.0f%% | <=10m %.0f%% | <=20m %.0f%%'
      % (np.median(d), 100 * np.mean(d <= 5), 100 * np.mean(d <= 10), 100 * np.mean(d <= 20)))
np.save('/tmp/dreich_phase5b_heads.npy', np.column_stack([E, Nn]))
